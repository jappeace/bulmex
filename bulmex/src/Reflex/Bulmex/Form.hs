...
actionForm
  ...
  -> m c
actionForm actF monM = form $ \onEnter -> do
  rec
    let
      action    = formDat ^. _2
      sendEvt   = onEnter <> (() <$ noNothing (preview _PostDefault <$> action))
      startSpin = sendEvt <> (() <$ noNothing (preview _Loading <$> action))
      stopSpin =
...
spinState start stop = accumDyn (const id) FormStateRest
  $ leftmost [FormStateSpinning <$ start, FormStateRest <$ stop]
...
loadAttr FormStateSpinning =
  Map.fromList [("class", "is-loading"), ("disabled", "1")]
