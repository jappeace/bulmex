...
(sleep 1 && touch src/src.cabal) & # makes it go the first time
...
while nix-shell --pure -p pkgs.inotify-tools --run  "inotifywait -r -e modify -e create -e attrib ./src ./default.nix ./makefile";
do
